...
# [확률적 경사하강법]
# 훈련 데이터에서 샘플을 하나씩 꺼내서 손실 함수의 경사를 따라 최적의 모델을 찾는 알고리즘
# 손실함수(loss function):
#   - 이진분류: 로지스틱회귀, 이진 크로스엔트로피
#   - 다중분류: 크로스엔트로피
#   - 회귀문제: 평균 제곱 오차 손실 함수(MSE)
# 비용함수(cost function):
#   - 훈련 세트에 있는 모든 샘플에 대한 손실함수의 합
#   - 일반적으로 구분하지 않고 사용    
# 에포크:
#   - 확률적 경사하강법에서 전체 샘플을 모두 사용하는 한 번의 반복   
...
#훈련 세트 점수는 에포크가 진행될수록 증가하지만 테스트 세트 점수는 
#어느 순간 감소하기 시작한다.
...
print(matplotlib.__version__) # 3.7.2
print(missingno.__version__)  # 0.5.2
...
# 상관관계
train_corr_df = train.loc[:, ['Pclass','Age','SibSp','Parch','Fare','Survived']]
...
# 결측값 대체
data_num['Age'] = data_num['Age'].fillna(data_num['Age'].mean())       # 평균값
...
# pc = cm[1,1] / (cm[0,1] + cm[1,1])
pc = TP / (TP + FP)
print(cm)
print("precision(정밀도): ", round(pc, 4))

# rc = cm[1,1] / (cm[1,0] + cm[1,1])
rc = TP / (TP + FN)
print("recall(재현율): ", round(rc,4))

F1 = 2 * (pc * rc) / (pc + rc)
print("F1 score(재현율): ", round(F1,4))
...
# 리셋인덱스를 하면 기존 인덱스는 컬럼으로 이동
# 인덱스가 새로운 번호호 부여
# 컬럼으로 이동한 인덱스 컬럼을 삭제

# 중복 인덱스 조정
data.reset_index(inplace=True)
data.drop('index', axis=1, inplace=True) # 컬럼삭제
...
# sns.barplot(x='FamilySize', y='Survived', hue='Pclass', estimator=np.sum,  data=data[data['TrainSplit']=='Train'])
sns.barplot(x='FamilySize', y='Survived', hue='Pclass', estimator=np.std,  data=data[data['TrainSplit']=='Train'])

plt.show()

#%%
sns.countplot(x='FamilySize', hue='Pclass', data=data[data['TrainSplit']=='Train'])
plt.show()
...
print("개별 Fold의 AUC 점수: ", np.round(auc_scores, 4))   # [0.8295 0.8876 0.8937]
print("평균 AUC 점수: ", np.round(np.mean(auc_scores), 4)) # 0.8703 

xgb_model.fit(X_train, y_train)
print("스코어(결정계수): ", xgb_model.score(X_train, y_train)) # 0.9124579124579124
...
# loss: 'log_loss' 손실함수(로지스틱 회귀)
sgd_model = SGDClassifier(loss='log_loss', max_iter=100, random_state=42)
sgd_auc_scores = cross_val_score(sgd_model, X_train, y_train, cv=3, scoring='roc_auc')
print("# 확률적 경사하강법")
print("개별 Fold의 AUC 점수: ", np.round(sgd_auc_scores, 4))   # [0.7851 0.853  0.8743]
print("평균 AUC 점수: ", np.round(np.mean(sgd_auc_scores), 4)) # 0.8374
sgd_model.fit(X_train, y_train)
y_test_pred_sgd = sgd_model.predict(X_test)

print("스코어(결정계수): ", sgd_model.score(X_train, y_train)) # 0.7789001122334456

#%%

# XGBoost vs SGDClassifier 비교하여
# 일치여부 확인
pd.Series(y_test_pred == y_test_pred_sgd).value_counts()
