...
# API 
# ## 8.1 웹 API의 이해
# application programming interface을 이용하면 html과 다르게 다이렉트로 웹
# 서버에 원하는 데이터를 요청하고 응답받을 수 있다.
#
# ### 8.1.1 웹 API의 데이터 획득 과정
# 클라이언트가 서버에 필요한 데이터를 요청하면 서버가 응답해 데이터를 보낸다.
...
# #### JSON 데이터 형식
# JSON 이 데이터 수집에서 웹포멧으로 가장 많이 사용되고 있음.
# dictionary의 기본적인 틀과 유사함.
# Json은 텍스트 형식이 아닌 바이너리 데이터 형태의 형식이다.

# #### JSON 데이터 변환 및 데이터 추출
# XML 은 코드안에서 표현을 할 수 가있음 구조에 맞춰서
# 작성하다 보니 텍스트가 크고 형식이 복잡함, 반면에 JSON은 코드안에서 name(key), value
# 값으로 이루어져 있다보니 정의가 명확하여 배열의 구조와 길이가 간단하다.

# JavaScript Object Notation = JSON 객체 표기
# [8장: 341페이지]
...
#dict -> json (indent 3칸 들여쓰기) 가독성이 높아짐.
json_data = json.dumps(python_dict, indent=3, sort_keys=True, ensure_ascii=False)  #ensure_accii=False -> 한글표현
print(json_data)
# sort_keys 키를 기준으로 정렬화 시킨 것임.
