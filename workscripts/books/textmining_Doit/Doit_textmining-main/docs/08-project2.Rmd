...
```{r ref.label="wide14", echo=F, highlight.output=c(4, 15, 16, 19)}
```
---
...
##### 중립 트윗 제외하고 다시 만들기
```{r,  p_tweet_polar_line, fig.width=7, fig.height=6, out.width='38%'}
# 중립 트윗 제외
...
---
```{r ref.label="p_tweet_polar_line", echo=F, fig.width=7, fig.height=6, out.width='70%', fig.align='center'}
```
---
...
```{r p_tweet_polar, fig.show='hide', echo=F}
# 색깔 목록 생성
col_polar <- c("#619CFF", "#F8766D")
...
```{r ref.label="p_tweet_polar", echo=F, fig.width=7, fig.height=6, out.width='70%', fig.align='center'}
```
---
...
```{r p_tweet_polar_area, fig.show='hide'}
ggplot(tweet_polar, aes(x = date, y = n, fill = sentiment)) +
  geom_area(position = "dodge", alpha = 0.7) +
  facet_wrap(~ candidate, nrow = 2, scales = "free_x") +
...
---
```{r ref.label="p_tweet_polar_area", echo=F, fig.align='center', fig.width=7, fig.height=6, out.width='70%', fig.align='center'}
```
---
...
.scroll-box-24[
```{r R.options=list(tibble.width = 60)}
# 두 글자 이상 한글 단어 추출
...
```{r, p_top10_pos_bar, fig.width=7, fig.height=5, out.width='30%'}
...
```{r ref.label="p_top10_pos_bar", echo=F, fig.width=7, fig.height=6, out.width='70%', fig.align='center'}
```
---
...
---
```{r ref.label="p_top10_pos", echo=F, fig.width=7, fig.height=6, fig.align='center', out.width="70%"}
```
---
...
```{r p_top10_pos_pop_raw, fig.width=7, fig.height=5, out.width="50%"}
...
```{r ref.label="p_top10_pos_pop_raw", echo=F, fig.width=7, fig.height=6, out.width='70%', fig.align='center'}
```
...
```{r p_top10_pos_pop, fig.show='hide'}
ggplot(top10_pos, aes(x = word,
                      y = log_odds_ratio,
                      col = candidate)) +
...
```
---
```{r ref.label="p_top10_pos_pop", echo=F, fig.width=7, fig.height=6, fig.align='center', out.width="70%"}
```
---
...
```{r, echo=F, R.options=list(crayon.enabled = T)}
...
```{r echo=F, highlight.output=c(21:24, 26)}
log_odds_neg %>%
  group_by(candidate = ifelse(log_odds_ratio > 0, "이낙연", "이재명")) %>%
  slice_max(abs(log_odds_ratio), n = 15, with_ties = F) %>%
  select(word)
...
---
```{r ref.label="p_top10_neg", echo=F, fig.width=7, fig.height=6, out.width='70%', fig.align='center'}
```
---
...
```{r p_top10_neg_pop, fig.show='hide', echo=F}
# 단어 순서 지정해 factor 타입으로 변환
top10_neg <- top10_neg %>%
  ungroup() %>%
  mutate(word = reorder(word, log_odds_ratio))
...
```{r ref.label=" p_top10_neg_pop", echo=F, fig.width=7, fig.height=6, out.width='70%', fig.align='center'}
```
